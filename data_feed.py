...
class DataFeedModule:
    ...
    def get_realtime_data(self):
        rates = mt5.copy_rates_from_pos(self.symbol, self.timeframe, 0, 60)  # حداقل 60 کندل
        if rates is None or len(rates) < 60:
            logger.warning(f"داده لحظه‌ای کافی نیست ({len(rates)} کندل). از داده‌های تاریخی استفاده می‌شود.")
            conn = sqlite3.connect(self.db_name)
            df = pd.read_sql_query(
                f"SELECT * FROM historical_data WHERE symbol='{self.symbol}' ORDER BY time DESC LIMIT 60", conn)
            conn.close()
            return df
        df = pd.DataFrame(rates)
        df['time'] = pd.to_datetime(df['time'], unit='s')
        df['symbol'] = self.symbol
        return df[['time', 'open', 'high', 'low', 'close', 'tick_volume', 'symbol']]
