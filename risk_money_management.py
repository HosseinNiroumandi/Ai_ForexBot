...
class RiskMoneyManagement:
    ...
    def update_drawdown(self, current_equity):
        """
        به‌روزرسانی و مدیریت Drawdown
        :param current_equity: ارزش فعلی حساب
        :return: ضریب کاهش حجم (در صورت نیاز)
        """
        self.current_drawdown = (self.account_balance - current_equity) / self.account_balance
        if self.current_drawdown > self.max_drawdown:
            logger.warning(f"Drawdown بیش از حد مجاز: {self.current_drawdown:.2%}")
            return 0.5  # کاهش 50% حجم معاملات
        return 1.0  # بدون تغییر
    ...
    def manage_trade(self, signal, other_open_trades=[]):
        """
        مدیریت ریسک و تولید پارامترهای معامله
        :param signal: سیگنال از AI Trading Core (1: خرید، 2: فروش، 0: نگه‌داری)
        :param other_open_trades: لیست جفت‌ارزهای در حال معامله
        :return: دیکشنری شامل حجم، استاپ‌لاس و تیک‌پروفیت
        """
        if signal == 0:
            return None

        # بررسی همبستگی با معاملات باز
        for trade_symbol in other_open_trades:
            if self.check_correlation(trade_symbol):
                logger.warning("معامله به دلیل همبستگی بالا لغو شد.")
                return None

        # بارگذاری داده‌ها و محاسبه ATR
        df = self.load_data()
        ...
        account_info = mt5.account_info()
        current_equity = account_info.equity if account_info else self.account_balance
        drawdown_factor = self.update_drawdown(current_equity)
        lot_size *= drawdown_factor

        # تنظیم پارامترهای معامله
        current_price = mt5.symbol_info_tick(self.symbol).ask if signal == 1 else mt5.symbol_info_tick(self.symbol).bid
        stop_loss = current_price - (stop_loss_pips * 0.0001) if signal == 1 else current_price + (
                    stop_loss_pips * 0.0001)
        take_profit = current_price + (take_profit_pips * 0.0001) if signal == 1 else current_price - (
        ...
        logger.info(f"پارامترهای معامله: {trade_params}")
        return trade_params
