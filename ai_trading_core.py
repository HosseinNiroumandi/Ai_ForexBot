...
class TradingEnv(gym.Env):
    def __init__(self, core):
        ...
        self.position = 0  # 0: بدون پوزیشن، 1: لانگ، -1: شورت
    ...
    def _get_state(self):  # حذف آرگومان df
        try:
            if len(self.df) < self.core.lookback * 2:
                logger.warning(f"تعداد کندل‌ها ({len(self.df)}) برای محاسبه حالت کافی نیست.")
                return np.zeros((self.core.lookback, 6))
            df_tech = self.core.technical_analysis(self.df.tail(self.core.lookback * 2))
            if df_tech.empty or len(df_tech) < self.core.lookback:
                logger.warning(f"داده‌های کافی برای محاسبه حالت وجود ندارد (تعداد کندل‌ها: {len(df_tech)}).")
                return np.zeros((self.core.lookback, 6))
            state = df_tech[['close', 'rsi', 'ema_short', 'ema_long', 'macd', 'volume_ema']].values[-self.core.lookback:]
            return state
    ...
    def predict_trend(self, df):
        try:
            if len(df) < self.lookback * 2:
                logger.warning(f"تعداد کندل‌ها ({len(df)}) برای پیش‌بینی روند کافی نیست.")
                return 0

            df_tech = self.technical_analysis(df.tail(self.lookback * 2))
            if df_tech.empty or len(df_tech) < self.lookback:
                logger.warning(f"داده‌های کافی برای پیش‌بینی روند وجود ندارد (تعداد کندل‌ها: {len(df_tech)}).")
                return 0
